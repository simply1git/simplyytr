...
export async function getOrDownloadMusic(musicDir: string): Promise<string | null> {
    ...
    try {
        const outputPath = path.join(musicDir, 'default_lofi.mp3');
...
export function addEditorJob(youtubeId: string) {
...
async function processEditorQueue() {
    if (isEditing || editorQueue.length === 0) return;

    isEditing = true;
    const youtubeId = editorQueue.shift()!;

    try {
        systemState.setTask(`Editing video for Shorts: ${youtubeId}`);
        ...
        const inputPath = path.resolve(__dirname, '../..', trend.localFilePath);
        const outputPath = path.resolve(__dirname, '../../storage/processed', `${youtubeId}_shorts.mp4`);
...
export function processVideoForShorts(inputPath: string, outputPath: string, settings: any, musicPath: string | null): Promise<void> {
    return new Promise((resolve, reject) => {
        // Use relative path for FFmpeg on Windows to avoid 'C:/' drive letter colon parsing issues
        const fontPath = 'assets/font.ttf';
        
        let yPos = '(h/2)';
        if (settings.overlayPosition === 'TOP') yPos = '(h/5)';
        if (settings.overlayPosition === 'BOTTOM') yPos = '(h*4/5)';
