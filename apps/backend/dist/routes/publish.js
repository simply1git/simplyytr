...
const CREDENTIALS_PATH = path_1.default.resolve(__dirname, '../../client_secret.json');
function getOAuth2Client() {
    ...
    const creds = JSON.parse(fs_1.default.readFileSync(CREDENTIALS_PATH, 'utf8')).web || JSON.parse(fs_1.default.readFileSync(CREDENTIALS_PATH, 'utf8')).installed;
    return new googleapis_1.google.auth.OAuth2(creds.client_id, creds.client_secret, creds.redirect_uris[0] || 'http://localhost:3001/api/publish/oauth2callback');
}
// 1. Generate Auth URL
...
let globalTokens = null; // Storing in memory for MVP. Should be in DB.
...
router.post('/force-next', async (req, res) => {
    try {
        ...
        const videoPath = path_1.default.resolve(__dirname, '../..', trend.processedFilePath);
        const { title, description } = await (0, aiRewriter_1.generateViralMetadata)(trend.topic, settings);
        const videoId = await (0, puppeteerUploader_1.uploadViaPuppeteer)(videoPath, title, description);
